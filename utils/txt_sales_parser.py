...
#         self.address_regex = re.compile(r"\d{1,5}\s+[A-Za-z0-9 .,'\-]+(?:Avenue|Street|Drive|Road|Boulevard|Lane|Bay|Crescent|Place)")
...
#             price_matches = re.findall(r"\$\d{1,3}(?:,\d{3})*", block)
#             list_price, sold_price = (parse_currency(price_matches[0]), parse_currency(price_matches[1])) if len(price_matches) >= 2 else (0.0, 0.0)
...
#     # Define locally or import if moved to a config
#     known_neighborhoods = [
...
#     df = parser.parse_folder("pdf_uploads")
#     print(df.head())
#     df.to_csv("parsed_csv/from_txt_parser.csv", index=False)
